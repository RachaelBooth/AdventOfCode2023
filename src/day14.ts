    ...
    while (y < platform.sizeY) {
        ...
        while (x < platform.sizeX) {
            if (platform.roundedRocks[y].has(x)) {
                let z = 0;
                while (y - z - 1 >= 0 && !newRoundedRocks[y - z - 1].has(x) && !platform.cubeRocks[y - z - 1].has(x)) {
                    z++;
                }
...
function tiltSouth(platform: platform): platform {
    ...
    while (y >= 0) {
        ...
        while (x < platform.sizeX) {
            if (platform.roundedRocks[y].has(x)) {
                let z = 0;
                while (y + z + 1 < platform.sizeY && !newRoundedRocks[y + z + 1].has(x) && !platform.cubeRocks[y + z + 1].has(x)) {
                    z++;
                }
                newRoundedRocks[y + z].add(x);
            }
...
function tiltWest(platform: platform): platform {
    ...
    while (x < platform.sizeX) {
        ...
        while (y < platform.sizeY) {
            ...
            if (platform.roundedRocks[y].has(x)) {
                let z = 0;
                while (x - z - 1 >= 0 && !newRoundedRocks[y].has(x - z - 1) && !platform.cubeRocks[y].has(x - z - 1)) {
                    z++;
                }
...
function tiltEast(platform: platform): platform {
    ...
    while (x >= 0) {
        ...
        while (y < platform.sizeY) {
            ...
            if (platform.roundedRocks[y].has(x)) {
                let z = 0;
                while (x + z + 1 < platform.sizeX && !newRoundedRocks[y].has(x + z + 1) && !platform.cubeRocks[y].has(x + z + 1)) {
                    z++;
                }
                newRoundedRocks[y].add(x + z);
            }
