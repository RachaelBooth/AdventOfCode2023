...
async function readBricks(inputFileName: string): Promise<brick[]> {
    ...
    for (const line of lines) {
        ...
        const vector = {
            x: start.x == end.x ? 0 : (end.x - start.x) / Math.abs(end.x - start.x),
            y: start.y == end.y ? 0 : (end.y - start.y) / Math.abs(end.y - start.y),
            z: start.z == end.z ? 0 : (end.z - start.z) / Math.abs(end.z - start.z),
        }
        // We're told it's single direction...
        const length = Math.abs(end.x - start.x + end.y - start.y + end.z - start.z)
        const ref = `${c}`;
        c++;
    ...
    while (unsettledBricks.length > 0) {
        ...
        for (const brick of unsettledBricks) {
            ...
            if (brick.start.z == 1 || (brick.vector.z < 0 && brick.start.z - brick.length == 1)) {
                ...
                const fallenLocations = locations.map(l => add(l, { x: 0, y: 0, z: -1 })).map(locationAsString);
                ...
                } else if (fallenLocations.every(l => !occupied.has(l) || locationsAsStrings.includes(l))) {
                  // Can fall
...
export async function solve2(inputFileName: string): Promise<number> {
    ...
    // Would fall when B removed if some z where only ancestor is B
    const ancestors = new Dictionary<ListDictionary<string>>();
    for (const ref of brickRefs) {
        const a = new ListDictionary<string>();
        const brick = settled.get(ref);
        let supporters = brick.restingOn;
        while (supporters.size > 0) {
            const next = new Set<string>();
            for (const supporter of supporters) {
                if (supporter != "Floor") {
                    const b = settled.get(supporter);
                    const bz = zLocations(b);
                    for (const z of bz) {
                        a.addTo(z, b.ref);
                    }
                    for (const s of b.restingOn) {
                        next.add(s);
                    }
                }
            }
    ...
    let totalFalls = 0;
    for (const ref of brickRefs) {
        const zs = zLocations(settled.get(ref));
        for (const f of brickRefs) {
            if (f != ref) {
                for (const z of zs) {
                    const zAncestors = ancestors.get(f).get(z);
                    if (zAncestors.length == 1 && zAncestors[0] == ref) {
                        totalFalls++;
                    }
                }
            }
