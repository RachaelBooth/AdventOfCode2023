...
async function getAnswer(day: string, part1: boolean, inputFile: string): Promise<void> {
  const fileLocation = `./day${day}`;
  const solver = await import(fileLocation);
  const inputFileName = inputFile ?? `day${day}`;
  const answer = part1 ? await solver.solve1(inputFileName) : await solver.solve2(inputFileName);
